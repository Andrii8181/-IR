...
def set_window_icon(win):
    # як у тебе: базово шукаємо icon.ico поруч із main.py
    try:
...
# ============================================================
# Table formatting for report (tabs)
# ============================================================
def fmt_num(x, nd=3):
    try:
        if x is None:
...
def brown_forsythe_from_groups(groups_dict):
    """
    Brown–Forsythe (robust Levene with median), single-factor over provided groups.
    Returns (F, p).
    """
    try:
        z_groups = []
        ...
        ssb = sum(len(z) * ((np.mean(z) - grand) ** 2) for z in z_groups)
        ssw = sum(np.sum((z - np.mean(z)) ** 2) for z in z_groups)

        dfb = k - 1
        dfw = n - k
        msb = ssb / dfb if dfb > 0 else np.nan
        msw = ssw / dfw if dfw > 0 else np.nan
        F = msb / msw if (msw is not None and msw > 0) else np.nan
...
def ols_fit(y, X):
    """
    OLS via lstsq. Returns beta, resid, sse, df_resid, rank.
    """
    y = y.reshape(-1, 1)
    if X.size == 0:
...
def type3_anova(y, X_full, term_slices, term_testable_names):
    """
    Type III SS by comparing reduced model (term removed) vs full.
    Returns dict term -> (SS, df), plus (SSE_full, df_resid).
    """
...
# -------------------------
# Build X for Shapiro (design-aware residuals)
# -------------------------
...
# ============================================================
# CLD letters (deterministic, supports multiple letters)
# ============================================================
def cld_multi_letters(names, center_values, sig_matrix):
    # сортуємо за спаданням середніх (для стабільності)
    order = sorted(
        names,
        key=lambda n: (-(center_values.get(n, -1e18) if center_values.get(n) is not None else -1e18), str(n))
    )

    letters = {}
    alphabet = list("abcdefghijklmnopqrstuvwxyz")
    groups_for_letter = []  # list[set]
...
def duncan_sig_matrix(names, groups1, ms_error, df_error, alpha=0.05):
    """
    Sig-matrix for Duncan MRT, based on sorted means.
    Pair (i,j): r = j-i+1, alpha_r, qcrit(r, df), Tukey-Kramer SE.
    """
    sig = {}
    if studentized_range is None:
        for i in range(len(names)):
...
def posthoc_for_labels(method, names, groups1, ms_error, df_error, alpha=0.05):
    """
    Єдина точка входу:
      - rows (таблиця парних, якщо метод її вимагає)
      - sig_matrix (для CLD)
      - headers (для звіту)
      - note (примітка)
    """
...
def build_factor_block_boxplot_png(long, factor_keys, levels_by_factor, letters_factor, indicator, units):
    ...
    # помірна сітка (не густа, але читабельна)
    ax.grid(True, axis="y", linestyle="--", linewidth=0.6, alpha=0.6)

    # місце під підписи факторів знизу
    ...
    for i, x in enumerate(x_positions):
        ...
        try:
            # верхній вус: 2*i+1
            wy = float(np.max(whiskers[2 * i + 1].get_ydata()))
        except Exception:
            wy = float(np.nanmax(np.array(data_list[i], dtype=float)))
        if math.isnan(wy):
            continue
...
# ============================================================
# Main app class (UI stable) + analysis
# ============================================================
class SADTk:
    ...
    # -------------------------
    # Dialog: choose method based on Shapiro (residuals)
    # -------------------------
    def choose_method_window(self, p_norm):
        ...
        normal = (p_norm is not None) and (not math.isnan(float(p_norm))) and (float(p_norm) > 0.05)

        if normal:
            msg = ("Дані експерименту відповідають принципам нормального розподілу\n"
                   "за методом Шапіра-Вілка (на залишках моделі).")
    ...
    # -------------------------
    # Plot window: ONLY Save (PNG). No Copy button.
    # -------------------------
    ...
    # ============================================================
    # MAIN analyze() — FULL (parametric full methods + new Shapiro + letters)
    # ============================================================
    def analyze(self):
        ...
        # -------------------------
        # Shapiro on model residuals (design-aware)
        # -------------------------
        try:
        ...
        # -------------------------
        # Factor groups and means
        # -------------------------
        factor_groups = {f: {k[0]: v for k, v in groups_by_keys(long, (f,)).items()} for f in self.factor_keys}
        factor_means = {
            f: {lvl: float(np.mean(np.array(arr, dtype=float))) if len(arr) else np.nan
        ...
        # -------------------------
        # Letters (CLD): factors always for parametric; variants only for CRD/RCBD
        # -------------------------
        ...
        if not nonparam:
            ...
            for f in self.factor_keys:
                ...
                _, sig_tmp, _, _ = posthoc_for_labels(method, lvl_names, groups_f, MS_e, df_e, alpha=ALPHA)
                means_tmp, _ns_tmp = means_ns_from_groups(lvl_names, groups_f)
                letters_map = cld_multi_letters(lvl_names, means_tmp, sig_tmp)

                for lvl in lvls:
                    letters_factor[f][lvl] = letters_map.get(str(lvl), "")

            # variant letters + pairwise table only for CRD/RCBD
            if design != "Split-plot":
                MS_e, df_e = model["MS_error"], model["df_error"]
                ...
                means_v, _ns_v = means_ns_from_groups(v_names, groups1)
        ...
        if not (W is None or (isinstance(W, float) and math.isnan(W))):
            seg.append(("text",
        ...
        if nonparam:
            ...
            self.show_report_segments(seg, plot_png_path=plot_png, plot_meta={"design": design})
            return
        ...
        if method == "lsd":
            ...
            if design == "Split-plot":
                for f in self.factor_keys:
                    ...
                    lsdv = lsd_value(ALPHA, df_e, MS_e, n_eff) if not math.isnan(n_eff) else np.nan
                    lsd_rows.append([f"Фактор {f}", fmt_num(MS_e, 4), str(int(df_e)), fmt_num(n_eff, 2), fmt_num(lsdv, 4)])
        ...
        for i, k in enumerate(variant_order):
            ...
            if design == "Split-plot":
                ...
                letter = letters_variants.get(name, "") or "-"
        ...
        if design == "Split-plot":
            ...
            # ---------- Pairwise table for Tukey/Duncan/Bonferroni ----------
