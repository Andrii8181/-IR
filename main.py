...
class EditableTreeview(ttk.Treeview):
    """Treeview з можливістю редагування клітинок"""
    ...
    def on_double_click(self, event):
        ...
        # Створюємо Entry поверх клітинки
        ...
        def save_edit(event=None):
            ...
            self.set(row_id, column=column, value=new_value)
            entry.destroy()
            self.entry_popup = None
...
class SADApp:
    def __init__(self):
        ...
        tk.Label(self.root, text="Двічі клікніть на клітинку — вводьте вручну\nАбо Ctrl+V з Excel", 
        ...
        tk.Label(footer, text="© 2025 Чаплоуцький А.М., УНУС", fg="#666", bg="#f5f7fa").pack(side="right", padx=30)
    ...
    def open_window(self, factors):
        ...
        # Додаємо 10 порожніх рядків для зручності
        for _ in range(10):
            self.tree.insert("", "end", values=[""]*20)

        # Підказка
        tk.Label(win, text="Двічі клікніть на клітинку — вводьте вручну | Ctrl+V — вставка з Excel", 
        ...
        tk.Button(btns, text="З Excel", bg="#2196F3", fg="white", width=18,
                  command=lambda: self.load_excel()).pack(side="left", padx=8)
    ...
    def paste_from_clipboard(self):
        try:
            ...
            messagebox.showinfo("Готово", f"Вставлено {len(df)} рядків з буфера обміну")
        except Exception as e:
            messagebox.showwarning("Помилка", f"Не вдалося вставити:\n{e}\nСпробуйте скопіювати з Excel ще раз")
    ...
    def clean_data(self):
        ...
        for child in self.tree.get_children():
            ...
            for v in values:
                try:
                    row.append(float(v)) if v not in ("", None) else row.append(np.nan)
                except:
                    row.append(np.nan)
            if any(not np.isnan(x) for x in row):
                data.append(row)
        if not data:
            raise ValueError("Немає числових даних!")
        df = pd.DataFrame(data).select_dtypes(include=[np.number])
        if df.empty or df.shape[1] < 2:
            raise ValueError("Потрібно мінімум 2 стовпці з числами!")
    ...
    def test_normality(self, data):
        flat = data.flatten()
        if len(flat) < 8:
            return "Даних замало для тесту"
        stat, p = stats.shapiro(flat)
        return f"Шапіро-Вілк: W = {stat:.4f}, p = {p:.5f} → {'Нормальний' if p > 0.05 else 'НЕ нормальний'}"
    ...
    def perform_analysis(self, factors):
        ...
        if "НЕ нормальний" in normality and not messagebox.askyesno("Попередження", f"{normality}\n\nПродовжити?"):
            return

        if values.shape == (6, 4):
            report = self.two_way_classic(values, normality)
        else:
            report = f"Ви вставили {values.shape[0]}×{values.shape[1]} даних\n{normality}\n\nПоки що підтримується лише класична схема 6×4 (2×3×4)\nІнші схеми — у наступній версії."
